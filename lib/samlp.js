...
module.exports.auth = function(options) {
  ...
  options.getSessionIndexFromRequest = options.getSessionIndexFromRequest || function(req){ return req.session && req.session.id; };
  ...
  return function (req, res, next) {
    ...
    function execute (postUrl, audience, req, res, next) {
      ...
      var sessionId = opts.getSessionIndexFromRequest(req);
      if (!user) return res.send(401);

      opts.audience = audience;
      opts.postUrl = postUrl;
      opts.sessionIndex = sessionId;
